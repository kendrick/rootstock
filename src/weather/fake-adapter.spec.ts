...
describe('fakeObservations', () => {
	it('resolves to exactly the list handed in, same contents and order', async () => {
		...
		await expect(fetch({ location: { latitude: 30.27, longitude: -97.74, timeZone: 'America/Chicago' }, now: new Date('2026-09-10T12:00:00.000Z') })).resolves.toEqual(observations);
	});

	it('ignores its arguments: different location/now yield the same list', async () => {
		const fetch = fakeObservations(observations);

		const first = await fetch({ location: { latitude: 30.27, longitude: -97.74, timeZone: 'America/Chicago' }, now: new Date('2020-01-01T00:00:00.000Z') });
		const second = await fetch({ location: { latitude: 51.5, longitude: -0.12, timeZone: 'Europe/London' }, now: new Date('2030-06-15T00:00:00.000Z') });
	...
	it('round-trips an empty list as an empty list', async () => {
		...
		await expect(fetch({ location: { latitude: 0, longitude: 0, timeZone: 'UTC' }, now: new Date('2026-09-10T00:00:00.000Z') })).resolves.toEqual([]);
	});
});
