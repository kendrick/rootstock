 ...
 * guarantee is the part a stranger cannot infer from looking: what the evidence
 * line under each Task is, and that the model can reword a Task but never add
 * one. `Purpose` below already introduces the yard and claims every Task
 * names its Rule and evidence, so this must not say either again.
 *
...
export function FirstVisitBand(): ReactElement | null {
	...
	return (
		<aside
			aria-label="New here"
			className="flex flex-col gap-1 border-2 border-foreground px-4 pt-3 print:hidden sm:flex-row sm:items-center sm:gap-x-6 sm:py-1"
		>
