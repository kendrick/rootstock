...
function formatReading(value: number, unit: 'F' | 'mm' | 'percent'): string {
	...
	return unit === 'percent' ? `${value}%` : `${value}mm`;
}
...
function windowStanding(rule: Extract<Rule, { kind: 'window' }>, asOf: string): { waitingOn: string; daysAway: number | null } {
	...
	const start = monthDayToDayOfYear(rule.start);
	const end = monthDayToDayOfYear(rule.end);
	const open = start <= end ? today >= start && today <= end : today >= start || today <= end;

	if (open) {
		return { waitingOn: `Open through ${formatMonthDay(rule.end)}`, daysAway: 0 };
	}

	const away = start > today ? start - today : 365 - today + start;

	return { waitingOn: `Opens ${formatMonthDay(rule.start)}`, daysAway: away };
}
...
export function standingFor(rule: Rule, plan: Plan): RuleStanding {
	...
	if (rule.kind === 'guard') {
		...
		return {
			...
			band: 'guard',
			inCurrentPlan: acted,
			waitingOn: acted ? 'Acted on this Plan' : 'Holds work back; creates none',
			daysAway: null,
		};
	}

	if (fired) {
		return { rule, band: 'fired', inCurrentPlan: true, waitingOn: 'Produced work this week', daysAway: 0 };
	}
	...
	if (rule.kind === 'threshold') {
		...
		const wants = `${rule.comparison === 'gte' ? 'at least' : 'at most'} ${formatReading(rule.value, rule.unit)}`;
		...
		return {
			...
			waitingOn: observed === null
				...
				? `No reading in the Artifact's window; wants ${wants}`
				: `Last read ${formatReading(observed.value, rule.unit)}, wants ${wants}`,
			daysAway: null,
		};
	}
	...
	return {
		...
		band: 'waiting',
		inCurrentPlan: false,
		waitingOn: min === max ? `Every ${min} days` : `Every ${min}–${max} days`,
		daysAway: null,
	};
}
