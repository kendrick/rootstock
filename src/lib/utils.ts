 ...
 * without this list `text-title text-foreground` merged to `text-foreground`
 * and the size silently fell back to 16px. A new size token goes in both
 * places, and `utils.spec.ts` lists them again.
 */
const twMerge = extendTailwindMerge({
	extend: {
		theme: {
			text: ['wordmark', 'display', 'title', 'body', 'label', 'evidence', 'detail'],
		},
	},
});
